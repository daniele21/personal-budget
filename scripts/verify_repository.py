#!/usr/bin/env python3
"""Structural checks for Aura's adopted repo-template-sw baseline."""
...
def main() -> int:
    ...
    for name in CORE_SKILLS:
        if not (root / "skills" / name / "SKILL.md").is_file():
            errors.append(f"missing core skill: skills/{name}/SKILL.md")

    try:
        baseline = json.loads((root / ".engineering" / "baseline.json").read_text(encoding="utf-8"))
    ...
    if baseline:
        ...
        if standard.get("revision") != "8aa95d10254846e7d63f4bd5c60d61b18d21060c":
            errors.append("baseline standard.revision must match canonical 0.9.2")
        ...
        for rel in ("README.md", "AGENTS.md", "docs/architecture.md", "SECURITY.md"):
            ...
            if path.is_file():
                ...
                for marker in PLACEHOLDERS:
                    if marker in text:
                        errors.append(f"unresolved adopter placeholder {marker} in {rel}")
